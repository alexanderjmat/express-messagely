...
class User {
  ...
  static async messagesFrom(username) {
    const results = await db.query("SELECT m.id, u.first_name, u.last_name, u.phone, m.body, m.sent_at, m.read_at FROM messages AS m JOIN users AS u ON m.to_username = u.username WHERE from_username = $1", [username])
  ...
  static async messagesTo(username) {
    const results = await db.query("SELECT m.id, u.first_name, u.last_name, u.phone, m.body, m.sent_at, m.read_at FROM messages AS m JOIN users AS u ON m.from_username = u.username WHERE from_username = $1", [username])
    return results.rows.map(m => ({
      id: m.id,
      from_user: {
